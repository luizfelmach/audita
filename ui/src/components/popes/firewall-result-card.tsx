...
import { FirewallDocument } from "@/types/search";

interface FirewallResultCardProps {
  document: FirewallDocument & { id: string };
  hashSigner: string | null;
  hashSignerLoading: boolean;
  hashStorage: string | null;
  hashStorageLoading: boolean;
  onSearchDHCP: (dst_ip: string, dst_port: string) => void;
}

export function FirewallResultCard({
  document,
  hashSigner,
  hashSignerLoading,
  hashStorage,
  ...
  return (
    <Card className="transition-shadow duration-200 shadow-none">
      <CardContent className="p-4">
          ...
          <div className="space-y-2">
            ...
            <div className="bg-slate-50 rounded-lg p-3">
              <div className="text-sm font-mono">
                {document.src_ip}:{document.src_port}
              </div>
              <div className="text-xs text-gray-500">
                Mapped: {document.src_mapped_ip}:{document.src_mapped_port}
              </div>
            </div>
          </div>

          <div className="space-y-2">
            <div className="text-xs font-medium text-gray-600">Destination</div>
            <div className="bg-slate-50 rounded-lg p-3">
              <div className="text-sm font-mono">
                {document.dst_ip}:{document.dst_port}
              </div>
              <div className="text-xs text-gray-500">
                Mapped: {document.dst_mapped_ip}:{document.dst_mapped_port}
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="mb-3">
          <Button
            onClick={() => onSearchDHCP(document.dst_ip, document.dst_port)}
            className="w-full bg-gradient-to-r from-green-600 to-emerald-600"
          >
            <ArrowRight className="w-4 h-4 mr-2" />
            Search DHCP for {document.dst_ip}:{document.dst_port}
          </Button>
        </div>
